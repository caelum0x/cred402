...
for both the contract page and the exact **install deploy transaction**), the real
on-chain deploy transactions we made (`deploys.transactions.json`, pulled from
cspr.live), the deployer account, per-contract event counts, and a
filterable/​pausable event stream. Powered by `GET /api/chain` (the deployment
manifest with explorer + deploy-tx links) and `GET /api/export/events.csv` (the
