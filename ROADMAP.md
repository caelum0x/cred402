...
| **LP deposit preview** | ✅ `lib/services/lp_deposit_preview.ts` — the lender's counterpart to the credit simulator: previews resulting pool share, utilization, and projected annual yield for a hypothetical deposit (no funds moved). `/v1/credit/lp-preview?deposit_cspr=` + console Credit Pool preview widget + MCP tool + ts-sdk (4 tests) |
...
| **Bulk agent registration** | ✅ `POST /v1/agents/batch` — register up to 100 agents in one call (operator fleet onboarding) with per-item ok/error so a duplicate doesn't abort the batch + ts-sdk (2 tests) |
...
| **Marketplace statistics** | ✅ `lib/services/marketplace_stats.ts` — supply-side stats for the service marketplace: listings by category & pricing strategy, price range, average seller reputation, top sellers. `/v1/analytics/marketplace` + console Marketplace-tab stats card + GraphQL + ts-sdk (2 tests) |
...
| **Go + Rust + Python SDK refresh** | ✅ brought `sdk/go`, `sdk/rust` and `sdk/python` back to parity with the newest surfaces (health/readiness/score-trend/multichain/similar/credit-cost/compare/batch-register on agents; categories/movers/disputes/x402/risk-alerts/yield/config analytics) — all build/import clean and verified live |
| **Protocol config (rulebook)** | ✅ `lib/services/protocol_config.ts` — self-documenting live parameters: fee schedule, credit gates (min reputation, max exposure), and the full reputation-tier perk table. `/v1/config` + console Developer-tab rulebook + GraphQL `config` + MCP tool + ts-sdk (1 test) |
| **Bureau lifecycle demo** | ✅ `npm run demo:bureau` (`scripts/demo_bureau.ts`) — a narrated end-to-end walk: readiness → what-if → offer → accept → draw cost → health → benchmark → compare → trend → multichain → protocol analytics. Living documentation + smoke test against a live server |
| **`bureau` CLI refresh** | ✅ added `health/market/x402/disputes/config` subcommands so the CLI reaches every analytics surface (human + `--json`) |
| **Deploy/dev fixes** | ✅ added `frontend/src/vite-env.d.ts` (types `import.meta.env` for the deployed cross-origin `streamUrl` config — unblocks `tsc -b`/Vercel build) and extended the Vite dev proxy to `/v1`·`/graphql`·`/report`·`/metrics` so every bureau surface works under `npm run dev` |
| **Contracts dependency pin** | ✅ pinned `zeroize 1.8.1` · `zeroize_derive 1.4.2` · `indexmap 2.7.1` · `getrandom 0.3.3` in `contracts/Cargo.lock` — crates.io had published `edition2024`-requiring releases the pinned mid-2024 nightly can't parse; the 14 Odra contracts compile again |
...
| **Safe-draw advisor** | ✅ `lib/services/safe_draw.ts` — the largest additional draw that keeps a line at/above a target health factor (default 1.5×), bounded by line headroom and pool free liquidity, with the binding constraint named. `/v1/agents/:id/safe-draw` + GraphQL + MCP tool + ts-sdk (4 tests) |
...
| **Agent dossier (one-call)** | ✅ `lib/services/agent_dossier.ts` — integrator snapshot bundling tier + health + readiness + benchmark percentile + reputation momentum + credit line + operator in one round-trip (each section reuses the standalone service, so it never drifts). `/v1/agents/:id/dossier` + GraphQL + MCP tool + ts-sdk (2 tests) |
| **Similar agents recommender** | ✅ `lib/services/similar_agents.ts` — "you might also consider": same-category peers ranked by closeness in standing (reputation + credit) and overall discovery strength. `/v1/agents/:id/similar` + AgentDetail "see also" section + GraphQL + MCP tool + ts-sdk (3 tests) |
