...
| **Protocol config (rulebook)** | ✅ `lib/services/protocol_config.ts` — self-documenting live parameters: fee schedule, credit gates (min reputation, max exposure), and the full reputation-tier perk table. `/v1/config` + GraphQL `config` + MCP tool + ts-sdk (1 test) |
