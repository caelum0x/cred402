...
import { EvmSatelliteVault } from "./EvmSatelliteVault.js";

/**
 * EvmAdapter — an EVM satellite (e.g. Base, eip155:8453). It settles x402 payments
 * locally, mirrors address bindings, and executes credit through an
 * {@link EvmSatelliteVault} gated by Casper-issued Credit Authorization Notes.
 */
export class EvmAdapter implements ChainAdapter {
  ...
  constructor(
    ...
    readonly vault: EvmSatelliteVault,
  ...
  async bindAgentAddress(input: AddressBindingEnvelope): Promise<TransactionResult> {
    ...
    const tx_hash = keccak256(`bind:${input.external_address}:${input.agent_id}`);
  ...
  async submitReceipt(input: UniversalReceiptEnvelope): Promise<TransactionResult> {
    ...
    // A real ReceiptOutbox logs the full canonical receipt; we carry the envelope
    // on the event so the Casper-root relayer can rebuild and anchor it from logs.
  ...
  async submitEvidence(input: EvidenceAttestationEnvelope): Promise<TransactionResult> {
    const tx_hash = keccak256(`evidence:${input.evidence_hash}:${input.agent_id}`);
