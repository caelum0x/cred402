...
import { CosmosSatelliteVault, cosmosTxHash } from "./CosmosSatelliteVault.js";

/**
 * CosmosAdapter — a Cosmos SDK / CosmWasm satellite (e.g. cosmoshub-4, Osmosis,
 * Neutron). It settles x402 payments locally via IBC-transferred stablecoins,
 * mirrors Casper-rooted address bindings, and executes credit through a
 * {@link CosmosSatelliteVault} gated by Casper-issued Credit Authorization Notes.
 * Casper remains the identity/reputation/policy root; Cosmos provides execution
 * and liquidity.
 */
export class CosmosAdapter implements ChainAdapter {
  ...
  constructor(
    ...
    readonly vault: CosmosSatelliteVault,
  ...
  async getCapabilities(): Promise<ChainCapabilities> {
    // CosmWasm contracts + IBC settlement; ~6s Tendermint finality (single block).
    return { smartContracts: true, x402Settlement: true, creditExecution: true, finalitySeconds: 6 };
  }
  ...
  async bindAgentAddress(input: AddressBindingEnvelope): Promise<TransactionResult> {
    ...
    const tx_hash = cosmosTxHash(`bind:${input.external_address}:${input.agent_id}`);
  ...
  async submitEvidence(input: EvidenceAttestationEnvelope): Promise<TransactionResult> {
    const tx_hash = cosmosTxHash(`evidence:${input.evidence_hash}:${input.agent_id}`);
