...
///
/// This interface is mirrored verbatim from `contracts/evm/src/interfaces/ICasperSigVerifier.sol`
/// so the Flare satellite verifies the same Casper policy signatures the EVM satellite does.
/// The two Foundry projects are self-contained (no cross-project imports), so the shared
/// interface is duplicated rather than imported across project roots.
